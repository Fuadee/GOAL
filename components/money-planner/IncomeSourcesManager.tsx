...
type IncomeFormState = {
  ...
  status: 'stable' | 'unstable' | 'building' | 'future';
  count_in_total: boolean;
};

const defaultIncomeForm: IncomeFormState = {
  name: '', type: 'active', gross_amount: '', direct_cost: '0', category: 'real', status: 'stable', count_in_total: true
};
  ...
  return (
    <article className="theme-card p-5">
        ...
        <table className="theme-table">
          ...
          <tbody>
            {incomeSources.map((item) => {
              const gross = Number(item.gross_amount ?? item.expected_income ?? item.actual_income ?? 0);
              const cost = Number(item.direct_cost ?? 0);
              const net = Number(item.net_amount ?? gross - cost);
              const category = item.category ?? 'real';
              const countInTotal = item.count_in_total ?? item.is_counted_in_real_income ?? category !== 'future';
              return <tr key={item.id} className="text-[color:var(--text-primary)]"><td className="py-2">{item.name}</td><td>{currency.format(gross)}</td><td>{currency.format(cost)}</td><td>{currency.format(net)}</td><td>{category}</td><td>{item.stability ?? 'stable'}</td><td>{countInTotal ? 'yes' : 'no'}</td><td><div className="flex items-center gap-2"><button type="button" onClick={() => { setEditingIncomeSourceId(item.id); setIncomeForm({ name: item.name, type: item.type, gross_amount: String(gross), direct_cost: String(cost), category, status: item.stability ?? (category === 'future' ? 'future' : 'stable'), count_in_total: countInTotal }); setIncomeMsg(null); }} className="theme-button-secondary rounded-md px-2 py-1 text-xs">Edit</button><button type="button" disabled={isIncomeDeletePending || isIncomePending} onClick={() => { if (!window.confirm('Delete this income source? This cannot be undone.')) return; setIncomeMsg(null); startIncomeDeleteTransition(async () => { const result = await deleteIncomeSourceAction(item.id); setIncomeMsg(result.message); if (result.success) { if (editingIncomeSourceId === item.id) clearIncomeEditor(); router.refresh(); } }); }} className="rounded-md border border-rose-300/35 bg-rose-500/10 px-2 py-1 text-xs text-rose-200 transition hover:bg-rose-500/20 disabled:opacity-50">Delete</button></div></td></tr>;
            })}
          </tbody>
        </table>
      </div>
      <form className="mt-4 grid gap-2 md:grid-cols-2" action={(formData) => { setIncomeMsg(null); if (editingIncomeSourceId) formData.set('id', editingIncomeSourceId); startIncomeTransition(async () => { const result = await createIncomeSourceAction(formData); setIncomeMsg(result.message); if (result.success) { clearIncomeEditor(); router.refresh(); } }); }}>
        ...
        <select name="category" value={incomeForm.category} onChange={(e) => setIncomeForm((p) => ({ ...p, category: e.target.value as IncomeFormState['category'] }))} className="theme-input"><option value="real">current income</option><option value="growing">building income</option><option value="future">future income</option></select>
