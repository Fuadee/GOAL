...
export function InnovationProcessSection({ innovationId, currentFocus, steps }: InnovationProcessSectionProps) {
  ...
  return <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    <div className="space-y-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
      ...
      {!currentFocus ? <p className="text-slate-600">ยังไม่มี Current Focus</p> : <article className="space-y-3"><p className="text-lg font-semibold text-slate-900">{currentFocus.title}</p><span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${stepStatusStyles[currentFocus.status]}`}>{STATUS_LABEL[currentFocus.status]}</span><div className="flex justify-end"><select defaultValue={currentFocus.status} onChange={(e) => setStatus(currentFocus.id, e.target.value as InnovationStepStatus)} className="w-full max-w-[220px] rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs text-slate-700 sm:w-auto"><option value="todo">Todo</option><option value="waiting">Waiting</option><option value="in_progress">In Progress</option><option value="blocked">Blocked</option><option value="completed">Done</option></select></div></article>}
    </div>
    ...
    <section className="space-y-3"><h3 className="text-xl font-semibold text-slate-900">Process Board</h3>{groups.map((group) => {
      const content = <ul className="space-y-2">{group.steps.map((step) => <li key={step.id} className="rounded-xl border border-slate-200 bg-white p-3"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><p className="text-sm font-medium text-slate-900">{step.title}</p>{step.note ? <p className="text-xs text-slate-600">{step.note}</p> : null}<p className="text-[11px] text-slate-500">Updated {new Date(step.updated_at).toLocaleDateString()}</p></div><div className="sm:ml-auto"><select defaultValue={step.status} onChange={(e) => setStatus(step.id, e.target.value as InnovationStepStatus)} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-1 text-xs text-slate-700 sm:w-auto"><option value="todo">Todo</option><option value="waiting">Waiting</option><option value="in_progress">In Progress</option><option value="blocked">Blocked</option><option value="completed">Done</option></select></div></div></li>)}</ul>;
