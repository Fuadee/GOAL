...
export function InnovationCard({ innovation, isCurrent = false }: InnovationCardProps) {
  ...
  return (
    <article className={`space-y-3 rounded-2xl border bg-white/5 p-4 backdrop-blur transition duration-300 ${isCurrent ? 'border-amber-300/45 shadow-[0_0_16px_rgba(251,191,36,0.14)]' : 'border-white/10 hover:border-white/20'}`}>
      <div className="space-y-1.5">
        <div className="flex items-start justify-between gap-3">
          <h3 className="text-base font-bold text-white">{innovation.title}</h3>
        ...
        <p className="line-clamp-2 text-xs leading-relaxed text-slate-300">{innovation.description || 'No description yet.'}</p>
        <p className="text-xs text-slate-400">{stateMeta.description}</p>
      ...
      <Link href={`/innovation/${innovation.id}`} className="inline-flex rounded-xl bg-white/10 px-3 py-2 text-xs font-semibold text-white transition hover:bg-white/20">
        Open details
      </Link>
    </article>
  );
