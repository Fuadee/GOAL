...
const stateStyles: Record<InnovationDerivedState, string> = {
  idea: 'bg-slate-500/20 text-[#64748B] border border-slate-400/30',
...
export function InnovationCard({ innovation, isCurrent = false }: InnovationCardProps) {
  ...
  return (
    <article className={`space-y-4 rounded-2xl border bg-white/5 p-5 backdrop-blur transition duration-300 hover:scale-[1.01] ${isCurrent ? 'border-amber-300/50 shadow-sm' : 'border-[#DDE3D5] hover:border-[#DDE3D5]'}`}>
      <div className="space-y-2">
        <div className="flex items-start justify-between gap-3">
          <h3 className="text-lg font-bold text-[#1E293B]">{innovation.title}</h3>
        ...
        <p className="text-sm leading-relaxed text-[#64748B]">{innovation.description || 'No description yet.'}</p>
        <p className="text-xs text-[#64748B]">{stateMeta.description}</p>
      ...
      <div className="grid grid-cols-2 gap-3 text-xs text-[#64748B] sm:grid-cols-3">
        <div>
          <p className="text-[#94A3B8]">Progress</p>
          <p className="font-semibold text-[#1E293B]">{innovation.progressPercent}%</p>
        </div>
        <div>
          <p className="text-[#94A3B8]">Steps</p>
          <p className="font-semibold text-[#1E293B]">{innovation.completedStepCount} / {innovation.stepTotal}</p>
        </div>
        <div className="col-span-2 sm:col-span-1">
          <p className="text-[#94A3B8]">Updated</p>
          <p className="font-semibold text-[#1E293B]">{formatTimestamp(innovation.updated_at)}</p>
        </div>
      </div>

      <p className="text-sm text-[#64748B]">
        Next: <span className="font-semibold text-[#1E293B]">→ {innovation.nextStep?.title ?? 'No pending step'}</span>
      </p>
        ...
        {(derivedState === 'idea' || derivedState === 'building') ? (
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-xl border border-[#DDE3D5] bg-white/70 px-3 py-2 text-xs text-[#1E293B]"
            />
            <button
              type="button"
              disabled={isPending}
              onClick={() => runAction(() => addInnovationStepAction(innovation.id, stepTitle))}
              className="rounded-full bg-[#EEF1EA]/20 px-3 py-2 text-xs font-semibold text-[#334155] disabled:opacity-50"
            >
      ...
      <Link href={`/innovation/${innovation.id}`} className="inline-flex rounded-full bg-white/10 px-4 py-2 text-xs font-semibold text-[#1E293B] transition hover:bg-white/20">
        Open details
      </Link>
    </article>
  );
