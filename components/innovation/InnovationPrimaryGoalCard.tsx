...
export function InnovationPrimaryGoalCard({ completedCount, targetCount, imageUrl }: InnovationPrimaryGoalCardProps) {
  const progress = Math.min(Math.max((completedCount / targetCount) * 100, 0), 100);
    ...
    <article className="grid overflow-hidden rounded-[24px] border border-blue-200 bg-gradient-to-br from-white via-blue-50/40 to-sky-100/60 shadow-[0_22px_52px_-38px_rgba(37,99,235,0.55)] md:grid-cols-[minmax(240px,0.8fr)_minmax(0,1.7fr)]">
      <div className="relative min-h-[210px] overflow-hidden bg-blue-100 md:min-h-full">
        {imageUrl ? (
          <Image
            src={imageUrl}
            alt="ภาพเป้าหมายสร้างแอปให้สำเร็จ 10 แอปจากบอร์ดวิสัยทัศน์"
            fill
            priority
          ...
          <div
            ...
            aria-label="ยังไม่มีภาพเป้าหมายด้านนวัตกรรมในบอร์ดวิสัยทัศน์"
          >
      ...
      <div className="flex min-w-0 flex-col justify-center p-5 sm:p-7 lg:p-8">
        <p className="text-sm font-semibold text-blue-700">เป้าหมายหลักด้านนวัตกรรม</p>
        <h1 className="mt-2 text-2xl font-semibold leading-tight text-slate-900 sm:text-3xl">สร้างแอปให้สำเร็จ 10 แอป</h1>
        <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600 sm:text-base">เปลี่ยนไอเดียให้กลายเป็นแอปที่ใช้งานได้จริง และสร้างให้สำเร็จตามเป้าหมายที่กำหนด</p>

        <div className="mt-6 rounded-2xl border border-blue-100 bg-white/80 p-4 shadow-sm sm:p-5">
          <div className="flex items-center justify-between gap-4">
            <span className="font-medium text-slate-700">ความคืบหน้าสู่เป้าหมาย 10 แอป</span>
          ...
          <div
            ...
            role="progressbar"
            aria-label="ความคืบหน้าสู่เป้าหมายสร้างแอปให้สำเร็จ 10 แอป"
            aria-valuemin={0}
            aria-valuemax={100}
          ...
          <p className="mt-3 text-sm text-slate-600">{completedCount} จาก {targetCount} แอป</p>
        </div>
      </div>
    </article>
  );
