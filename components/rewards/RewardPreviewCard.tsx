    ...
    return (
      <section className="pt-2 sm:pt-3">
        <article className="rounded-3xl border border-dashed border-amber-200/35 bg-gradient-to-br from-[#2a1f12]/95 via-[#1f1710]/95 to-[#140f09]/95 p-5 shadow-[0_18px_36px_-30px_rgba(251,191,36,0.45)]">
          ...
          <button type="button" onClick={onAddReward} className="mt-4 rounded-xl border border-amber-300/40 bg-amber-300/10 px-4 py-2.5 text-sm font-medium text-amber-100 transition hover:bg-amber-300/20">+ เพิ่ม Reward</button>
        </article>
      </section>
    );
  }
          ...
          <div className="relative flex flex-col gap-5 p-5 sm:p-6">
            <div className="space-y-5">
              <div className="flex items-start justify-between gap-3">
                <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-semibold ${isClaimed ? 'border-emerald-300/40 bg-emerald-300/20 text-emerald-100' : isLocked ? 'border-amber-300/35 bg-amber-300/20 text-amber-100' : 'border-amber-200/45 bg-amber-300/30 text-amber-50'}`}><span aria-hidden="true">{isClaimed ? '✅' : isLocked ? '🔒' : '🏆'}</span>{isClaimed ? 'รับรางวัลแล้ว' : isLocked ? 'Locked Reward' : 'Unlocked Reward'}</span>
                <div className="flex flex-wrap items-center justify-end gap-2 sm:gap-2.5">
                  <button type="button" onClick={onDeleteReward} className="min-h-9 rounded-lg border border-rose-300/20 bg-rose-400/5 px-3 py-1.5 text-xs font-medium text-rose-100/75 transition hover:border-rose-300/35 hover:bg-rose-400/15 hover:text-rose-50">ลบ reward</button>
                  <button type="button" onClick={onAddReward} className="min-h-9 rounded-lg border border-amber-200/20 bg-amber-300/5 px-3 py-1.5 text-xs font-medium text-amber-100/80 transition hover:border-amber-200/35 hover:bg-amber-300/15 hover:text-amber-50">แก้ไข reward</button>
                </div>
              </div>
...
function RewardImage({ imageUrl, title, missionTitle, isLocked }: { imageUrl?: string; title: string; missionTitle?: string; isLocked: boolean }) {
  if (!imageUrl) return <div className="relative flex h-[240px] items-center justify-center border-b border-white/10 bg-[#1a120b] md:h-full md:min-h-[300px] md:border-b-0 md:border-r md:border-white/10"><p className="text-sm text-amber-100/70">ยังไม่มีรูปภาพรางวัล</p></div>;
  return <div className="relative h-[250px] overflow-hidden border-b border-white/10 bg-black md:h-full md:min-h-[320px] md:border-b-0 md:border-r md:border-white/10"><Image src={imageUrl} alt="Mission reward image" fill className="object-cover object-center" sizes="(min-width: 768px) 60vw, 100vw" /><div className="absolute inset-0 bg-gradient-to-t from-[#1c1209]/55 via-[#1f140d]/15 to-transparent" /><span className="absolute left-4 top-4 rounded-full border border-amber-200/40 bg-amber-200/20 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-amber-50">🎁 Reward Moment</span>{isLocked ? <span className="absolute right-4 top-4 inline-flex items-center gap-1 rounded-full border border-amber-100/35 bg-black/35 px-3 py-1 text-xs font-medium text-amber-50">🔒 Locked</span> : null}<div className="absolute inset-x-0 bottom-0 p-4 sm:p-5"><p className="line-clamp-2 text-2xl font-semibold text-white sm:text-3xl">{title}</p><p className="mt-1 line-clamp-2 text-sm text-amber-50/90">Mission สำเร็จแล้ว · {missionTitle || 'ภารกิจปัจจุบัน'}</p></div></div>;
}
