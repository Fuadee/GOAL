...
function statusBadge(status: RunnerProgressStatus | undefined) {
  ...
  return 'border-slate-500/30 bg-slate-700/50 text-[#64748B]';
}
    ...
    <section className="space-y-6">
      ...
      <section className="grid gap-3 sm:grid-cols-2">
        <article className="premium-card">
          <p className="text-xs uppercase tracking-[0.18em] text-[#94A3B8]">Current Level</p>
          <p className="mt-2 text-xl font-semibold text-[#1E293B]">{currentLevel ? `Level ${currentLevel.level_number} · ${currentLevel.title}` : 'All levels passed'}</p>
        </article>
        <article className="premium-card">
          <p className="text-xs uppercase tracking-[0.18em] text-[#94A3B8]">Best Recent Attempt</p>
          <p className="mt-2 text-xl font-semibold text-[#1E293B]">
      ...
      <section className="premium-card">
        <h3 className="text-lg font-semibold text-[#1E293B]">Current Level Focus</h3>
        {currentLevel ? (
          <>
            <p className="text-[#64748B]">ภารกิจของวันนี้ชัดเจน: วิ่ง {currentLevel.distance_target_km} km แบบไม่หยุด</p>
            ...
            {currentLevel.latestAttempt ? (
              <div className="mt-3 rounded-xl border border-[#DDE3D5] bg-[#F6F7F4]/70 p-3 text-sm text-[#64748B]">
              ...
              <p className="mt-2 text-sm text-[#94A3B8]">เริ่มก่อน แล้วค่อยดูสถิติ</p>
            )}
          </>
        ) : (
      ...
      <section>
        <h3 className="mb-3 text-lg font-semibold text-[#1E293B]">Level Progression</h3>
            ...
            return (
              <article
                ...
                className={`rounded-2xl border p-4 ${
                  isCurrent
                    ? 'border-sky-300/50 bg-sky-500/10 shadow-sm'
                    : 'border-[#DDE3D5] bg-white/70'
                }`}
              >
                <div className="flex items-center justify-between">
                  <p className="text-sm font-semibold text-[#1E293B]">Level {level.level_number}</p>
                ...
                <p className="mt-2 text-lg font-semibold text-[#1E293B]">{level.title}</p>
                <p className="text-sm text-[#64748B]">Target {level.distance_target_km} km · {formatPace(level.pace_target_seconds_per_km)}</p>
                <p className="mt-1 inline-flex rounded-full border border-[#DDE3D5]/30 bg-[#EEF1EA]/10 px-2 py-0.5 text-xs text-[#334155]">No Stop Required</p>
      ...
      <article id="attempt-history" className="premium-card">
        <h3 className="text-lg font-semibold text-[#1E293B]">Attempt History</h3>
        {data.logs.length === 0 ? (
          <p className="mt-2 text-sm text-[#94A3B8]">No run logs yet. Start with Level 1 and record your first attempt.</p>
        ) : (
            ...
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                {data.logs.slice(0, 25).map((log) => (
                  <tr key={log.id} className="border-t border-[#DDE3D5] text-[#64748B]">
